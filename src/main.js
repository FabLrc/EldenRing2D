...
function shrine(){
 ...
 setModal(`<div class="eyebrow">LE DERNIER REFUGE</div><h2>La flamme se souvient.</h2><p>Santé et fioles restaurées. Les habitants du sanctuaire se relèvent.</p><div class="stat-line"><span>Fragments disponibles</span><span>${p.souls} ${icon('fragment')}</span></div>${upgradeButton('vigor','Vigueur +20 PV')}${upgradeButton('healing','Soin +14 PV')}<p>${prog.talisman?`${icon('talisman')} Talisman du souffle équipé.`:'Un talisman oublié repose dans le jardin à l’est.'}</p><button class="primary" id="leave-shrine">Repartir</button>`,'shrine');
