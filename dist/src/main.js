import {createGame,update,startAction,interact,nearby,respawn,upgrade,serialize,ROOMS,SHRINE,distance,clamp} from './core.js';
...
function victory(){save();bell();setModal(`<div class="eyebrow">LE SERMENT EST ROMPU</div><h2>Enfin, le silence.</h2><p>Le gardien s’effondre. Pour la première fois depuis des siècles, la cloche se tait. Une lumière pâle traverse les pierres.</p><div class="stat-line"><span>Chutes du pèlerin</span><span>${s.progress.deaths}</span></div><div class="stat-line"><span>Raccourci</span><span>${s.progress.shortcut?'Ouvert':'Non découvert'}</span></div><div class="stat-line"><span>Talisman du souffle</span><span>${s.progress.talisman?'Recueilli':'Non découvert'}</span></div><p>Merci d’avoir joué à cette première démo.</p><button class="primary" id="explore">Continuer à explorer</button><button class="secondary" id="again">Recommencer la démo</button>`,'victory');$('explore').onclick=resume;$('again').onclick=confirmRestart;}
